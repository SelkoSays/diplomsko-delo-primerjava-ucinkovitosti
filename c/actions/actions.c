...
static Int trend_block_size(const Args *args) {
    SInt jitter = (SInt)next_uniform(0, args->trend_jitter.as.i * 2) -
                  (SInt)args->trend_jitter.as.i;

    switch (args->size_trend.as.e) {
    case TREND_NONE:
        return next_random(args->min_size.as.i, args->max_size.as.i,
                           args->dist_param.as.f, args->distribution.as.e);
    case TREND_GROW: {
        ...
        block_size_tmp =
            clamp(args->min_size.as.i, args->max_size.as.i, block_size_tmp);
...
static inline SInt abs_int(SInt n) { return (n >= 0) ? n : -n; }
...
/// @param args
/// @return Block size
static Int get_block_size(const Args *args) {
    IntList l = args->size_list.as.il;
    if (l.count == 0) {
        return trend_block_size(args);
    }

    switch (args->size_mode.as.e) {
    case SIZE_LIST_MODE_EXACT:
        return random_wchoice(&l, &args->size_weights.as.il);
    case SIZE_LIST_MODE_NEAREST: {
        return get_closest(&l, trend_block_size(args));
    } break;
    default:
...
static SInt get_block_ttl(const Args *args) {
    ...
    case TTL_LIST:
        return (SInt)random_wchoice(&args->ttl_list.as.il,
                                    &args->ttl_weights.as.il);
    default:
        panic("Unknown ttl mode %d", args->ttl_mode.as.e);
    }
}

static inline bool should_alloc(const Pool *pool, const Args *args) {
    return (pool->items.count < pool->capacity) &&
           (uniform01() < args->alloc_freq.as.f);
}
...
void block_action(Pool *pool, const Args *args) {
    ...
    if (!alloc) {
        pool_del_block(pool, (Policy)args->policy.as.e);
    } else {
